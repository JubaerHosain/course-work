...
/*****Global variable and uitlity functions*****/
...
// Function to compute xor(is a keyword of cpp, that's why used Xor) between two strings
...
string DES(string text){ 
	...
	for(int i = 0; i < 16; i++) { 
		...
		for(int i = 0; i < 8; i++){ 
            ...
            // First last bit of each 6 length group indicates row number
      		string row1= xored.substr(i*6,1) + xored.substr(i*6 + 5,1);
      		int row = convertBinaryToDecimal(row1);
            // Four bit of middle indicates column number
...
string Decrypt(string ciphertext) {
    ...
    // Apply same algorithm just reversing the round keys
    string decrypted_text = DES(ciphertext);

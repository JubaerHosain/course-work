...
// independent writer thread
void *writer(void *socket_desc) {
    char buffer[MAXDATASIZE];
...
int main(int argc, char *argv[]) {
    // argv[1] = host_name / host_ip 
    // argv[2] = client_name

    if (argc < 3) {
        printf("Argument format [./filename hostname client_name]\n"); 
        exit(1);  
    }
    ...
    // write client_name to server
    if (write(sockfd, argv[2], strlen(argv[2])) < 0) {
        perror("ERROR writing client_name to socket"); 
        exit(0);  
    }

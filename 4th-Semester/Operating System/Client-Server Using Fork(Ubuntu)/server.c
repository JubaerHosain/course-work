...
int main(void)
{
    ...
    while(1) {
        ...
        printf("server: got connection from %s(Client-%d)\n",inet_ntoa(their_addr.sin_addr), client_no);
        ...
        if(pid == 0) {
            while(1) {
                ...
                printf("Message from Client-%d: %s",client_no,buffer);
                
                // exit from this particular process
